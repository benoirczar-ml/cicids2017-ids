...
DATA_GLOB = "/srv/work/datasets/cicids2017/processed/windows_1s_ts_v1/*.parquet"
...
holdout_attacks = ["Bot", "Web Attack  Brute Force", "PortScan", "Infiltration"]
...
FEATURES = [
    ...
    "min_pkt_len_min",
    "mean_down_up_ratio",
    "sum_total_fwd_packets",
    "sum_total_bwd_packets",
    "sum_total_fwd_bytes",
    "sum_total_bwd_bytes",
    "nunique_dst_ip",
    "nunique_dst_port",
...
if not files:
    raise SystemExit(f"No parquet files found at {DATA_GLOB}")

_df = pd.concat([pd.read_parquet(f) for f in files], ignore_index=True)
_df[FEATURES] = _df[FEATURES].replace([np.inf, -np.inf], np.nan).fillna(0.0)
...
out_path = os.path.join(REPORT_DIR, "anomaly_faiss_knn_windows_1s_ts_v1.json")

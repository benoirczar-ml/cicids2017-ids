...
if len(X_val_b) > 50_000:
    X_val_b = X_val_b.sample(n=50_000, random_state=42)
...
# benign val kNN distance (use distance to kth neighbor)
val_dist, _ = knn.kneighbors(X_val_cp)
val_scores = val_dist[:, -1]
...
for holdout_attack in holdout_attacks:
    ...
    X_test_cp = (test_df.values - mean) / std
    test_dist, _ = knn.kneighbors(X_test_cp)
    test_scores = test_dist[:, -1]
...
out_path = os.path.join(REPORT_DIR, "anomaly_knn_botnet_focus.json")

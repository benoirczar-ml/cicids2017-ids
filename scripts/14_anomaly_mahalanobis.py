...
for holdout_attack in holdout_attacks:
    ...
    # Sample benign for mean/cov
    benign_train = benign_df.sample(n=min(len(benign_df), 300_000), random_state=42)
    benign_val = benign_df.sample(n=min(len(benign_df), 150_000), random_state=7)

    X_train = benign_train.drop(columns=[label_col, label_bin]).values
    X_val = benign_val.drop(columns=[label_col, label_bin]).values
    ...
    # Covariance + inverse (add small ridge for stability)
    cov = cp.cov(X_train, rowvar=False)
    cov += cp.eye(cov.shape[0]) * 1e-3
    cov_inv = cp.linalg.inv(cov)

    # Score: Mahalanobis distance
    def mahalanobis(x):
        xm = x
        return cp.sqrt(cp.sum((xm @ cov_inv) * xm, axis=1))

    val_scores = mahalanobis(X_val)
    val_scores_t = cupy_to_torch(val_scores).float()

    thr_1e3 = threshold_by_benign_fpr(val_scores_t, 1e-3)
    thr_1e4 = threshold_by_benign_fpr(val_scores_t, 1e-4)
    ...
    X_test = test_df.drop(columns=[label_col, label_bin]).values
    y_test = test_df[label_bin].astype("int32")

    X_test = (X_test - mean) / std
    test_scores = mahalanobis(X_test)

    y_test_t = cupy_to_torch(y_test.to_cupy()).int()
    test_scores_t = cupy_to_torch(test_scores).float()

    res = {
        "holdout_attack": holdout_attack,
        "holdout_rows": int(len(attack_df)),
        "test_rows": int(len(test_df)),
        "benign_train_rows": int(len(benign_train)),
        "benign_val_rows": int(len(benign_val)),
        "thresholds": {"fpr_1e-3": thr_1e3, "fpr_1e-4": thr_1e4},
        "test_metrics@fpr_1e-3": eval_metrics(y_test_t, test_scores_t, thr_1e3),
        "test_metrics@fpr_1e-4": eval_metrics(y_test_t, test_scores_t, thr_1e4),
    }

    results.append(res)

out_path = os.path.join(REPORT_DIR, "anomaly_mahalanobis_benign_only.json")

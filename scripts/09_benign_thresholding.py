...
def threshold_by_benign_fpr(benign_probs, fpr_target):
    # threshold = quantile at 1 - fpr
    q = 1.0 - fpr_target
    if hasattr(torch, "quantile"):
        return float(torch.quantile(benign_probs, q).item())
    # fallback: sort
    sorted_probs, _ = torch.sort(benign_probs)
    idx = int(max(0, min(len(sorted_probs) - 1, round(q * (len(sorted_probs) - 1)))))
    return float(sorted_probs[idx].item())
...
for holdout_attack in holdout_attacks:
    ...
    # Build benign-only validation set (thresholding without attack leakage)
    ...
    X_val_benign = val_benign_df.drop(columns=[label_col, label_bin])

    # Test: holdout attack + benign (balanced)
...
out_path = os.path.join(REPORT_DIR, "drift_holdout_xgb_benign_thresholds.json")

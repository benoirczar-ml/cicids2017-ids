...
for holdout_attack in holdout_attacks:
    ...
    # Protocol-based thresholds from benign val
    val_benign_probs = cupy_to_torch(cp.asarray(bst.predict(xgb.DMatrix(val_benign_df.drop(columns=[label_col, label_bin]).values)))).float()
    val_benign_protocol = cupy_to_torch(val_benign_df["Protocol"].values).int()

    global_thresh = threshold_by_benign_fpr(val_benign_probs, 1e-3)

    proto_thresholds = {}
    for proto in torch.unique(val_benign_protocol):
        mask = val_benign_protocol == proto
        if mask.sum().item() < 500:
            continue
        proto_probs = val_benign_probs[mask]
        proto_thresholds[int(proto.item())] = threshold_by_benign_fpr(proto_probs, 1e-3)

    # Apply protocol thresholds to test
    test_protocol = cupy_to_torch(test_df["Protocol"].values).int()
    thr_tensor = torch.full_like(test_probs, global_thresh)
    for proto, thr in proto_thresholds.items():
        thr_tensor[test_protocol == proto] = thr
    ...
    res = {
        ...
        "thresholds": {
            "global_fpr_1e-3": global_thresh,
            "protocol_fpr_1e-3": proto_thresholds,
        },
        "test_metrics@0.5": eval_metrics(y_test_t, test_probs, 0.5),
        "test_metrics@global_fpr_1e-3": eval_metrics(y_test_t, test_probs, global_thresh),
        "test_metrics@protocol_fpr_1e-3": eval_metrics(y_test_t, test_probs, thr_tensor),
    }

    results.append(res)

out_path = os.path.join(REPORT_DIR, "drift_holdout_xgb_protocol_thresholds.json")
